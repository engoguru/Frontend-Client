...
import { fetchApparelProducts } from '../../store/slice/productSlice';
function HomeFeature2() {

  const dispatch = useDispatch();

  const { apparelProducts, totalCount, totalPages, loading, error } = useSelector((state) => state.product);
  useEffect(() => {
    dispatch(fetchApparelProducts({ productCategory:'Apparel' }));
  }, [dispatch]);
    ...
    <>
      ...
      <div className="w-full text-center">
        ...
        <div className="flex bg-red-100 border-b border-gray-500 justify-between items-center w-full py-4">
          <div>
            <p className="text-xs sm:text-sm md:text-base lg:text-lg xl:text-[21px] font-semibold text-black px-2 transition-all duration-300">Featured Apparel Products</p>
          </div>
